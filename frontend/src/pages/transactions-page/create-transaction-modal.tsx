...
export function CreateTransactionModal({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const transaction = {
      transaction_title: formData.get("transaction_title")?.toString().trim() || "",
      transaction_value: formData.get("transaction_value")?.toString().trim() || "",
      transaction_date: selectedDate || new Date(),
      transaction_description: formData.get("transaction_description")?.toString() || "",
      transaction_is_debit: isDebit,
      transaction_is_deposit: isDeposit,
    };

    if (!transaction.transaction_title || !transaction.transaction_value || !transaction.transaction_date) {
          ...
          <div className="flex flex-col w-full items-center space-y-3">
            ...
            >
              ...
              <span className="text-[#ffffff]/40 text-left w-40 flex-1 font-semibold">
                {selectedDate ? format(selectedDate, "dd/MM/yyyy") : "Data de vencimento"}
              </span>
            </button>
